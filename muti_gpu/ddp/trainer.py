...
def create_model(rank, args):
    ...
    try:
        ...
        # 确保CUDA设备已设置（在spawn模式下，每个进程都需要设置）
        # 注意：在setup()中已经设置过了，这里再次确认
        torch.cuda.set_device(rank)
        
        # 在创建模型之前，再次验证CUDA上下文是否正常工作
        # 这可以提前捕获CUDA版本不匹配的问题
        try:
            # 创建一个小的测试tensor来验证CUDA上下文
        ...
        # 在创建DDP之前，确保所有进程的CUDA上下文都已完全初始化
        # 1. 在GPU上执行一些操作来确保CUDA上下文完全初始化
        test_tensor = torch.zeros(10, 10, device=f'cuda:{rank}')
        _ = test_tensor @ test_tensor.T  # 矩阵乘法，更彻底地初始化CUDA
        torch.cuda.synchronize(rank)
        del test_tensor
        torch.cuda.empty_cache()
        
        # 2. 在所有进程间进行barrier，确保所有进程都准备好了
        import torch.distributed as dist
        dist.barrier()
        
        # 3. 再次验证CUDA是否在所有进程上都正常工作
        # 通过一个简单的allreduce操作来测试NCCL和CUDA的协同工作
        ...
        # 4. 使用DDP包装模型
        # 注意：DDP在初始化时会验证参数，这需要CUDA上下文正常工作
        # 使用find_unused_parameters=False来减少验证开销
        try:
            ddp_model = DDP(
                model, 
                device_ids=[rank], 
                output_device=rank,
                find_unused_parameters=False,  # 如果模型所有参数都被使用，设置为False可以提高性能
                broadcast_buffers=True,
                gradient_as_bucket_view=True  # 优化内存使用
            )
        except RuntimeError as ddp_error:
            error_msg = str(ddp_error)
            if 'driver version' in error_msg.lower() or 'cuda error' in error_msg.lower() or 'NCCL' in error_msg:
                ...
                raise RuntimeError(
                    f"[Rank {rank}] DDP初始化失败 - CUDA/NCCL错误\n"
                    f"错误详情: {error_msg}\n"
                    f"诊断信息:\n"
                    f"  - PyTorch CUDA版本: {torch.version.cuda}\n"
                    ...
                    f"5. 如果问题持续，尝试重新安装PyTorch: pip install torch --force-reinstall"
                ) from ddp_error
            raise
